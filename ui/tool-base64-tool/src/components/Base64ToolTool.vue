...
async function refreshData() {
  try {
    ...
    // 调用后端 API
    const result = await invoke('plugin:base64-tool|get_config')
    console.log('base64-tool config:', result)
    ...
    console.error('Failed to load base64-tool data:', err)
  } finally {
    loading.value = false
  }
}
...
async function performAction(action: string) {
  try {
    loading.value = true
    const result = await invoke('plugin:base64-tool|base64Tool_action', {
      data: action
    })
