...
async function refreshData() {
  try {
    ...
    // 调用后端 API
    const result = await invoke('plugin:hash-tool|get_config')
    console.log('hash-tool config:', result)
    ...
    console.error('Failed to load hash-tool data:', err)
  } finally {
    loading.value = false
  }
}
...
async function performAction(action: string) {
  try {
    loading.value = true
    const result = await invoke('plugin:hash-tool|hashTool_action', {
      data: action
    })
