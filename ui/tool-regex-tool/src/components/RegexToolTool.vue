...
async function refreshData() {
  try {
    ...
    // 调用后端 API
    const result = await invoke('plugin:regex-tool|get_config')
    console.log('regex-tool config:', result)
    ...
    console.error('Failed to load regex-tool data:', err)
  } finally {
    loading.value = false
  }
}
...
async function performAction(action: string) {
  try {
    loading.value = true
    const result = await invoke('plugin:regex-tool|regexTool_action', {
      data: action
    })
